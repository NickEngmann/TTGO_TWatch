...
1. **Native Unit Tests** — PlatformIO `pio test -e native`, all 97 non-LVGL tests
2. **LVGL Simulator Tests** — PlatformIO `pio test -e native_lvgl`, 7 headless render tests, screenshots uploaded as artifacts
3. **ESP32 Build Check** — PlatformIO `pio run -e esp32`, compile verification
...
- LVGL 7.x in repo (`src/lvgl/tests/` has broken `green` member access) — excluded via `build_src_filter`
- PlatformIO `lib_deps` (TFT_eSPI) downloaded on first build — cache in CI
